...
class FileRenameEditor:
    ...
    def setup_ui(self):
        ...
        prefix_suffix_frame.grid(row=0, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        # 说明文字
        help_label = ttk.Label(prefix_suffix_frame, 
                              text="前缀会添加到文件名开头，后缀会添加到扩展名之前",
                              font=("Arial", 9), foreground="gray")
        help_label.grid(row=1, column=0, columnspan=4, pady=(5, 0))
    ...
    def preview_rename(self):
        ...
        try:
            ...
            self.update_status(f"前缀: '{prefix}' (添加到文件名开头)\n")
            self.update_status(f"后缀: '{suffix}' (添加到扩展名之前)\n")
